...
def worker(
    ...
    for step in steps:
        ...
        try:
            ...
            for n in ngram_orders:
                filler = [np.nan] * seq_len
                data[f"mean_{n}_gram_loss"].append(filler)
                data[f"mean_{n}_gram_kl_div"].append(filler)
                data[f"mean_{n}_gram_jk_div"].append(filler)
            continue

        pile = iter(pile_ds)
        ngram_iters = {n: iter(ds) for n, ds in ngram_data.items()}

        running_means = defaultdict(lambda: torch.zeros(seq_len, dtype=torch.float32))
...
def main(ngram_path: Path, pile_path: Path, output_path: Path):
    ...
    for model_name, batch_size in (
        ...
        pd.DataFrame(data).to_csv(
            output_path / f"ngram_indices_{model_name}_{num_samples}.csv",
            index=False,
        )
