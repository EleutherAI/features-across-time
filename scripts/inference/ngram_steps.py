...
def get_mean_divergences(
    ...
    for n in ngram_orders:
        ...
        divergences.append(js_divergence_log_space(ngram_dists, logits).mean())

    return torch.stack(divergences)
...
def ngram_model_worker(
    ...
    for n in experiment.ngram_orders:
        div_labels.append(f"{n}-gram_logit_kl_div")
        div_labels.append(f"{n}-gram_logit_js_div")
    ...
    for step in steps:
        ...
        for i in range(num_iters):
            for n_index, n in enumerate(experiment.ngram_orders):
                ngram_sample = get_ngram_seq(n_index, i, experiment.batch_size).cuda().long()
            ...
            divergences = get_mean_divergences(
                sample,
                logits,
                ngram_model,
                experiment.d_vocab,
