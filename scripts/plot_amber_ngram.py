...
def main():
    ...
    def decrease_confidence_intervals(
        ...
        df[top_conf_col] = df[mean_col] + (
            (df[top_conf_col] - df[mean_col]) / np.sqrt(sample_size)
        )
        df[bottom_conf_col] = df[mean_col] - (
            (df[mean_col] - df[bottom_conf_col]) / np.sqrt(sample_size)
        )
        return df
    ...
    for label in labels:
        ...
        df[f"bottom_conf_{label}_bpb"] = (
            df[f"bottom_conf_{label}_loss"] * bpb_coefficient
        )
