...
def main():
    ...
    for label in labels:
        ...
        trace1 = go.Scatter(
            x=df["index"], y=df[f"mean_{label}_bpb"], mode="lines", name="Mean BPB"
        )
        trace2 = go.Scatter(
            x=df["index"], y=df[f"{label}_grad"], mode="lines", name="Gradient"
        )
        layout = go.Layout(
            title=f"Mistral {label} sequence BPB with gradient",
        ...
        trace1 = go.Scatter(
            x=filtered_df["index"],
            y=filtered_df[f"mean_{label}_bpb"],
            mode="lines",
            name="Mean BPB",
        )
        trace2 = go.Scatter(
            x=filtered_df["index"],
            y=filtered_df[f"{label}_grad"],
            mode="lines",
            name="Gradient",
        )
        layout = go.Layout(
