...
def plot(
    ...
    for i, model in enumerate(df["pretty_model_name"].unique()):
        ...
        fig.add_trace(
            go.Scatter(
                x=df_model["step"],
                y=df_model["kl_div"] * bpb_coefficient,
                mode="lines+markers",
    ...
    for model_name, pretty_model_name, num_chunks in model_metadata:
        ...
        for step in steps:
            ...
            counts_path = (
                ...
                / "lucia"
                / "finetune"
                / f"2-finetune_bigram_{model_name}_{num_samples}_{step}.pkl"
            )
            with open(counts_path, "rb") as f:
                learned_bigram_probs = torch.tensor(
                    pickle.load(f).toarray(), dtype=torch.float32
                )

                learned_bigram_probs[learned_bigram_probs.sum(axis=1) == 0] = 1 / 50_277
                assert np.all(
                    np.isclose(learned_bigram_probs.sum(axis=1), 1)
                    | np.isclose(learned_bigram_probs.sum(axis=1), 0)
                )
                learned_bigram_probs = learned_bigram_probs.flatten()
            print("opened")
            kl_divs.append(
                kl_divergence(learned_bigram_probs, flattened_data_bigram_probs).item()
            )
            filtered_kl_divs.append(
                kl_divergence(
                    learned_bigram_probs[non_zero],
                    flattened_data_bigram_probs[non_zero],
                ).item()
            )
            print(filtered_kl_divs[-1])

        model_df = pd.DataFrame(
            {"step": steps, "kl_div": kl_divs, "filtered_kl_div": filtered_kl_divs}
        )
