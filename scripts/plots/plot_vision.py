...
def main(debug: bool):
    ...
    for model_name, pretty_model_name, num_chunks in model_metadata:
        model_df = pd.read_csv(Path.cwd() / "vision.csv")
...
def plot(
    ...
    fig = make_subplots(
        ...
        shared_yaxes=True,
        subplot_titles=[
            "Mean shifted data across time",
            "Mean shifted maximum entropy data across time",
        ],
        horizontal_spacing=0.02,
        vertical_spacing=0.1,
    )

    for idx, loss in enumerate(["maxent_shifted_loss", "ds_shifted_loss"]):
        df[f"mean_{loss}_bpb"] = df[f"mean_{loss}_loss"] * bpb_coefficient
        df[f"mean_{loss}_bpb_bottom_conf"] = (
            df[f"bottom_conf_{loss}_loss"] * bpb_coefficient
        )
        df[f"mean_{loss}_bpb_top_conf"] = (
            df[f"top_conf_{loss}_loss"] * bpb_coefficient
        )
        ...
        for i, model in enumerate(df["pretty_model_name"].unique()):
            ...
            fig.add_trace(
                go.Scatter(
                    x=df_model["step"],
                    y=df_model[f"mean_{loss}_bpb_top_conf"],
                    fill=None,
                    mode="lines",
                    line=dict(width=0),
                    showlegend=False,
            ...
            fig.add_trace(
                go.Scatter(
                    x=df_model["step"],
                    y=df_model[f"mean_{loss}_bpb_bottom_conf"],
                    mode="lines",
                    line=dict(width=0),
                    fill="tonexty",
